			...
			<div className="flex justify-between items-start mb-2">
				...
				<div className="flex gap-1 flex-wrap">
					<button
						...
						className="cursor-pointer text-blue-600 hover:text-blue-800 text-xs px-2 py-1 rounded hover:bg-blue-100"
						aria-label={`Editar cumpleaños de ${birthday.name}`}
					>
						Editar
					</button>
					<button
						onClick={shareIndividualBirthday}
						className="cursor-pointer text-green-600 hover:text-green-800 text-xs px-2 py-1 rounded hover:bg-green-100"
						aria-label={`Copiar cumpleaños de ${birthday.name}`}
					>
						Copiar
					</button>
					<button
						onClick={() => onDelete(birthday.id)}
						className="cursor-pointer text-red-600 hover:text-red-800 text-xs px-2 py-1 rounded hover:bg-red-100"
						aria-label={`Eliminar cumpleaños de ${birthday.name}`}
					>
						Eliminar
					</button>
				</div>
			...
			<div className="space-y-2">
				...
				<p
					className={`font-medium ${
						isToday ? "text-yellow-700" : isSoon ? "text-blue-700" : "text-gray-700"
					}`}
				>
					{isToday
						? "🎉 ¡Es su cumpleaños HOY!"
						: `⏰ ${birthday.daysUntilBirthday} días para su cumpleaños`}
				</p>
			</div>
		</div>
	);
